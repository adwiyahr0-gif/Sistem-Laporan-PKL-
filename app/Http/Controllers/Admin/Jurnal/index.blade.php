<x-admin-layout>
    <div class="mb-8">
        <h1 class="text-2xl font-black text-slate-800">Validasi Jurnal Mahasiswa</h1>
        <p class="text-slate-500 font-medium">Tinjau dan setujui laporan kegiatan harian mahasiswa magang.</p>
    </div>
                ...
                <tr class="hover:bg-slate-50/30 transition-colors">
                    <td class="p-6">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 bg-indigo-100 text-[#4e5ecf] rounded-xl flex items-center justify-center font-black">
                                {{ strtoupper(substr($report->user->name, 0, 1)) }}
                            </div>
                            <div>
                                <p class="text-sm font-black text-slate-800">{{ $report->user->name }}</p>
                                <p class="text-[10px] text-slate-400 font-bold uppercase">{{ $report->user->universitas }}</p>
                            </div>
                        </div>
                    </td>
                    <td class="p-6">
                        ...
                        <div class="flex justify-center">
                            @if($report->status == 'pending')
                                <form action="{{ route('admin.jurnal.approve', $report->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="group flex items-center gap-2 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase transition-all shadow-lg shadow-emerald-200">
                                        <i class="fa-solid fa-check group-hover:scale-125 transition-transform"></i> Setujui
                                    </button>
                                </form>
                            @else
                                <div class="flex items-center gap-2 text-slate-300 font-black text-[10px] uppercase">
                ...
                <tr>
                    <td colspan="4" class="p-20 text-center">
                        <div class="flex flex-col items-center">
                            ...
                            <p class="text-slate-400 font-bold italic">Belum ada laporan masuk untuk divalidasi.</p>
                        </div>
                    </td>
                </tr>
                @endforelse
